...
if($isdefault==1&&$list){
	...
	$result = mysqli_query($conn, $sql);
	if($result==true){
		$sql = "UPDATE xz_receiver_address SET   user_id=$_SESSION[loginUid],receiver='$n',province='$province',city='$city',
	county='$county',address='$address',cellphone='$cellphone',is_default=$isdefault WHERE aid=$aid";
		$result = mysqli_query($conn, $sql);
		if($result==true){
